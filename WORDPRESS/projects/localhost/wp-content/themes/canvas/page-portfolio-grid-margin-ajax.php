<?php
/*
Template Name: Portfolio Grid Margin Ajax
*/
if (!post_password_required()) {
    ...
    <div class="filters">
        <div class="fl">
            <?php
            if (function_exists('gt3pb_showPortCatsMasonry')) {
                echo gt3pb_showPortCatsMasonry($selected_categories);
            }
            ?>
        </div>
    </div>
    <div class="fullscreen_block_grid_margin_ajax">
        <div class="grid_margin_ajax_isotope_block"> </div>
        <a href="<?php echo esc_js("javascript:void(0)");?>" class="load_more_posts"><?php _e('More Items', 'canvas') ?></a>
    </div>

    <script>
        jQuery(window).load(function () {
            jQuery(".grid_margin_ajax_isotope_block").isotope({
                layoutMode: 'fitRows',
    ...
    $GLOBALS['showOnlyOneTimeJS']['portfolio_grid_margin_ajax'] = '
    <script>
    var posts_already_showed = 0;
    jQuery(window).load(function () {
        gt3_get_posts_grid_margin_ajax("gt3_get_posts", "port", "' . gt3_get_theme_option("fw_port_per_page") . '", posts_already_showed, "portfolio_grid_margin_ajax", "'.$selected_categories.'");
    ...
    function gt3_get_posts_grid_margin_ajax(action, post_type, posts_count, posts_already_showed, template, selected_categories) {
    jQuery.post(gt3_ajaxurl, {
        ...
        .done(function (data) {
            ...
            jQuery(".grid_margin_ajax_isotope_block").isotope("insert", jQuery(data), function () {
                jQuery(".grid_margin_ajax_isotope_block").ready(function () {
                    jQuery(".grid_margin_ajax_isotope_block").isotope("reLayout");
                });
            });
        });
    }
